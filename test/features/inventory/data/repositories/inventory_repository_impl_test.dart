...
class FakeLocalDataSource implements InventoryLocalDataSource {
  ...
  @override
  Future<void> saveProduct(ProductModel product) async => products[product.id] = product;

  @override
  Future<ProductModel?> getProductById(String productId) async => products[productId];

  @override
  Future<void> deleteProduct(String productId) async => products.remove(productId);

  @override
  Stream<List<ProductModel>> watchProducts() => Stream.value(products.values.toList());

  @override
  Future<void> saveStockMutation(StockMutationModel mutation) async => mutations[mutation.id] = mutation;

  @override
  Stream<List<StockMutationModel>> watchMutationHistory(String productId) =>
      Stream.value(mutations.values.where((m) => m.productId == productId).toList());
  ...
  @override
  Future<void> saveSyncOperation(SyncOperationModel operation) async => syncOps[operation.id] = operation;
  ...
  @override
  Future<void> updateSyncOperation(SyncOperationModel operation) async => syncOps[operation.id] = operation;
}

// Fake backend — deterministically always fails, so we can reliably
// force the isFinal / rollback path instead of relying on the real
// MockBackendService's random ~20% failure chance.
...
void main() {
  test('rollback restores previous quantity after permanent sync failure', () async {
    ...
    // Arrange: product starts at quantity 10
    ...
    // Act 1: optimistic update to 15
    await repository.updateStock(productId: 'p1', newQuantity: 15, changedBy: 'tester');
    final afterOptimistic = await repository.getProductById('p1');
    expect(afterOptimistic!.quantity, 15); // optimistic write happened immediately

    // Act 2: run sync 3 times — matches maxRetries, forcing isFinal on the 3rd attempt
